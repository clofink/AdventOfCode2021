fetch('/Day4Input.txt').then(response => response.text()).then(
    function(result) {
        console.log("Day 4 Part 2:", solve(result.split('\n')));
            ...
            for (let board of bingoBoards) {
                for (let i = 0; i < drawnNumbers.length; i++) {
                    ...
                    if (checkBoardForWin(drawnSoFar, board)) {
                        ...
                        if (drawnSoFar.length > lastToSolve) {
                            lastToSolve = drawnSoFar.length;
                            lastScore = unmarkedTotal * numberCalled;
                        }
                        break;
                    };
                }
